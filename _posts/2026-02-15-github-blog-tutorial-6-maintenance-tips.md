...
> 포스트가 사이트에 보이지 않는 가장 흔한 원인이 바로 **미래 날짜**. front matter의 `date`가 현재 시각 이전인지 반드시 확인하세요!
{: .prompt-danger }

### 예약 포스트 사용하기

미래 날짜로 포스트를 예약하고 싶다면, GitHub Actions에 **매일 자동 빌드**를 설정하면 됩니다.
...
블로그 로딩 속도를 위해 이미지를 최적화하는 것이 좋습니다:
